...
function! s:DB_getDefault(name)
    ...
    elseif a:name ==# "SQLSRV_cmd_options"      |return (exists("g:dbext_default_SQLSRV_cmd_options")?g:dbext_default_SQLSRV_cmd_options.'':'-w 10000 -r -b -n -s "|"')
...
function! s:DB_SQLSRV_getListColumn(table_name)
    ...
    "let query = query . " order by c.colid"
...
function! dbext#DB_getListColumn(table_name, silent_mode, use_newline_sep ) 
    ...
    if use_newline_sep == 0
        ...
        " let col_list = substitute( col_list, ',\s*', "\n", 'g' )
        let col_list = substitute( col_list, '\s*', '', 'g' )
    endif
