...
" TODO: format column results so that the data types line up correctly
" TODO: Add a mssql specific syntax file
" TODO: Review static completion mode

" FIXED: Find a way to return tables/views/procedures even when not at beginning
"
" beginning of line
...
function! sqlcomplete#Complete(findstart, base)
    ...
    if a:findstart
        ...
        while start > 0
            if line[start - 1] =~ '\(\w\)'
                let start -= 1
			" if it's table completion, we want to break at a space, because
			" there may not be any prefix, or period but I still want a full
			" table list in that case
    ...
    elseif compl_type =~? 'column'
        ...
        if base =~ '\.'
            ...
            let table  = matchstr( base, '\zs\<\w\+\>\ze\.$' )
...
function! s:SQLCGetColumns(table_name, list_type)
	...
	let column_completion = []
	let longest_col = 0

	for table_col in table_cols
		let split_cols = split(table_col, '|')
		if len(split_cols) > 0
			if len(split_cols[0]) > longest_col
				let longest_col = len(split_cols[0])
			endif

			call add(new_table_cols, split_cols)
		endif
	endfor

    if a:list_type == 'csv' && !empty(table_cols)
		let temp_cols = []
		for c in new_table_cols
			call add(temp_cols, c[0])
		endfor
		let cols = join(temp_cols, ', ')
		let cols = s:SQLCAddAlias(table_name, table_alias, cols)
		let column_completion = [cols]
	else
		for c in new_table_cols
			let comp_data = ''
			let spaces = repeat(' ', (longest_col - len(c[0])) + 2)
			if c[2] != 'NULL'
				let comp_data = c[0] . spaces . "[" . c[1] . "(" . c[2] . ")]"
			else
				let comp_data = c[0] . spaces . "[" . c[1] . "]"
			endif

			call add(column_completion, {'word': c[0], 'abbr':comp_data, 'icase': 0})
		endfor
    endif
